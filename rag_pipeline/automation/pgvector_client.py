...
def _table_ref() -> str:
    ...
    (singular, changeset ``rexi:create-rag-chunks`` in baseline.sql); the
    pipeline runs with DB_SKIP_INIT_DDL so it never creates it. The name is
    overridable via PGVECTOR_TABLE only if RExI ever renames the table.
    """
